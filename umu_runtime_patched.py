"""
Patched umu_runtime.py (user-provided patched version). This file was included in the issue conversation.
"""
import os
import platform
import shlex
...
def setup_umu(
    ...
    _t0_setup = time.time()
    log.debug("setup_umu start: local=%s runtime=%s", local, runtime_ver)
    ...
    log.debug("has_runtime_installed=%s UMU_RUNTIME_UPDATE=%s", has_runtime_installed(local), os.environ.get("UMU_RUNTIME_UPDATE", ""))
    ...
    if not has_runtime_installed(local):
        ...
        _t_http = time.time()
        log.debug("About to fetch latest-public-beta.txt from '%s'...", url)
        try:
            resp = http_pool.request(HTTPMethod.GET.value, url)
        except Exception as e:
            log.debug("HTTP request raised exception: %s", e)
            log.debug("setup_umu end: total_elapsed=%.3fs", time.time() - _t0_setup)
            return
        log.debug("HTTP request done: status=%s elapsed=%.3fs", resp.status, time.time() - _t_http)
        if resp.status != HTTPStatus.OK:
        ...
        log.debug("_restore_umu completed: elapsed=%.3fs", time.time() - _t_restore)
        log.info("Using %s (latest)", runtime_ver[1])
        log.debug("setup_umu end: total_elapsed=%.3fs", time.time() - _t0_setup)
        return

    # Existing install: only contact network if updates are enabled.
    if os.environ.get("UMU_RUNTIME_UPDATE") == "0":
        log.info("%s updates disabled, skipping", runtime_ver[1])
        log.debug("setup_umu end: total_elapsed=%.3fs", time.time() - _t0_setup)
        return

    # updates enabled -> fetch latest-public-beta.txt and continue.
    url: str = f"https://{host}{endpoint}/latest-public-beta.txt"
    _t_http2 = time.time()
    log.debug("About to fetch latest-public-beta.txt from '%s'...", url)
    try:
        ...
        log.debug("setup_umu end: total_elapsed=%.3fs", time.time() - _t0_setup)
        return
    log.debug("HTTP request done: status=%s elapsed=%.3fs", resp.status, time.time() - _t_http2)
    if resp.status != HTTPStatus.OK:
        log.error("%s returned the status: %s", host, resp.status)
        log.debug("setup_umu end: total_elapsed=%.3fs", time.time() - _t0_setup)
        return
    ...
    _t_update = time.time()
    log.debug("About to call _update_umu: elapsed_since_setup=%.3fs", time.time() - _t0_setup)
    _update_umu(local, runtime_ver, version, session_pools)
    log.debug("_update_umu completed: update_elapsed=%.3fs total_elapsed=%.3fs", time.time() - _t_update, time.time() - _t0_setup)
